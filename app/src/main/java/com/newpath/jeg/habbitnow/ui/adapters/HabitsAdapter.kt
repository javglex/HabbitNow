...
import com.newpath.jeg.habbitnow.ui.viewholders.HabitItemViewHolder.Companion.ACTION_EDIT_HABIT
import com.newpath.jeg.habbitnow.ui.viewholders.HabitItemViewHolder.Companion.ACTION_EDIT_TIME
import java.util.*
...
class HabitsAdapter(private val mHomeViewModel: HomeViewModel): ListAdapter<MyHabit, HabitItemViewHolder>(
    ...
    override fun onBindViewHolder(holder: HabitItemViewHolder, position: Int) {
        ...
        holder.bind(habitItem) { result, action ->
            if (action==ACTION_EDIT_HABIT)
//                openItemMenu(context, holder, res)
                mHomeViewModel.onNavigateToEditHabit(result)
            else if (action==ACTION_EDIT_TIME)
                openTimePicker(context, holder, result)
        }
    }
